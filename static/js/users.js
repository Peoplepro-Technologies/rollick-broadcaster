...
document.getElementById('xlsx-input').addEventListener('change', async (ev) => {
  ...
  try {
    ...
    if (r.ok) {
      const parts = [`+${body.inserted} added`, `~${body.updated} updated`, `!${body.skipped} skipped`];
      let msg = `Import complete — ${parts.join(', ')}.`;
      if (body.errors && body.errors.length) {
        msg += ` First error: row ${body.errors[0].row} (${body.errors[0].reason}).`;
      }
      result.textContent = msg;
      result.hidden = false;
      setTimeout(() => location.reload(), 800);
    } else {
      result.textContent = 'Import failed: ' + (body.detail || r.status);
      result.hidden = false;
    }
  } catch (e) {
